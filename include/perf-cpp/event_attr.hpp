...
public:
     ...
     */
    EventGuard open_as_group_leader(std::variant<Cpu, Thread>, int cgroup_fd = -1);
    ...
    bool is_available_in(std::variant<Cpu, Thread> location) const
    {
        ...
        {
            Cpu cpu = std::get<Cpu>(location);
            return availability_ != Availability::PROCESS_MODE &&
                   (cpus_.empty() || cpus_.count(cpu));
        }
    }
...
public:
     ...
     */
    EventGuard open_child(EventAttr& child, std::variant<Cpu, Thread> scope, int cgroup_fd = -1);
